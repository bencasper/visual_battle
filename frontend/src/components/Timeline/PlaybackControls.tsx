...
export function PlaybackControls({
  ...
  return (
    <div className="flex items-center gap-1">
      <Tooltip content={t('playback.stepBack')}>
        <button onClick={onStepBack} disabled={!canStepBack} className={btn}>⏮</button>
      </Tooltip>

      <Tooltip content={isPlaying ? t('playback.pause') : t('playback.play')}>
        <button
      ...
      <Tooltip content={t('playback.stepForward')}>
      ...
      <div className="flex items-center gap-0.5 ml-2 border border-wiki-border rounded p-0.5 bg-wiki-parchment">
        <Tooltip content={t('playback.speed1x')}><button onClick={() => onSpeedChange(1)} className={speedBtn(1)}>1×</button></Tooltip>
        <Tooltip content={t('playback.speed2x')}><button onClick={() => onSpeedChange(2)} className={speedBtn(2)}>2×</button></Tooltip>
        <Tooltip content={t('playback.speed4x')}><button onClick={() => onSpeedChange(4)} className={speedBtn(4)}>4×</button></Tooltip>
      </div>
    </div>
  )
}
